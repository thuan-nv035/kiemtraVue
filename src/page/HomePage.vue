            ...
            <div class="mt-6">
              <div class="flex max-w-4xl px-10 py-6 mx-auto bg-white rounded-lg shadow-md"
                ...
                <div class="w-9/12">
                  ...
                  <a :href="facebook + listArticlesItem.url">share facebook</a>
                  <br>
                  <a :href="twitter + listArticlesItem.url">share twitter</a>
                  <br>
                  <button class="text-blue-500" @click = "bookMark(listArticlesItem.url)">Danh dau</button>
                </div>
              </div>
            </div>
...
export default {
  ...
  methods: {
    ...
    bookMark(url) {
      if (this.articles.find(article => article.url === url)) {
        alert('false!');
      } else {
        this.articles.push(this.listArticles.find(article => article.url === url));
        localStorage.setItem('articles', JSON.stringify(this.carts));
        alert('success!')
      }
    }
  },
  mounted() {
    ...
    if(localStorage.getItem('articles')){
      this.carts = JSON.parse(localStorage.getItem('articles'))
    }
  },
}
</script>
